...
class CustomerServiceTest {
  ...
  private final Clock testClock = Clock.fixed(
    Instant.parse("2020-12-22T10:15:30.00Z"),
    ZoneId.of("UTC")
  );
  ...
  void shouldCalculateAgeCorrectly(LocalDate birthDate, int expectedAge) {
    assertThat(service.calculateAgeOfCustomer(birthDate))
  ...
  void shouldCalculateFullYearsCorrectly(
    LocalDate inceptionDate,
    int expectedFullYears
  ) {
    assertThat(service.calculateFullYearsSinceInception(inceptionDate))
      .isEqualTo(expectedFullYears);
  }

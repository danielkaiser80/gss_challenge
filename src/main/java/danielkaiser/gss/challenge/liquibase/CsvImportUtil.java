...
public final class CsvImportUtil {
  ...
  public <T> List<T> loadObjectList(
    ...
    try (
      ...
      log.error(
        String.format(
          "Error occurred while loading object list from file %s",
          fileName
        ),
        e
      );
      throw new IllegalStateException(
        "Error occurred while loading object list from file " + fileName,
        e
      );
    }
  }

  private <T> List<T> loadObjectList(
    Class<T> type,
  ...
  private <T> List<T> loadObjectList(
    ...
    Reader reader,
    CsvSchema schema,
    @NonNull final CsvMapper mapper
  ) {
    ...
    try (MappingIterator<T> values = objectReader.readValues(reader)) {
      ...
      throw new IllegalStateException(
        "Error occurred while loading object list from stream.",
        e
      );
    }
  }
